...
### Questions / Risks
- Keeping all granules maximises fidelity but may impact processing time; monitor runtime and adjust if GitHub Actions approaches the timeout.
- Current fallback covers up to a two-hour delay; if outages last longer, do we still want the text-only post or should we add a manual rerun?
